...
If (Test-Path -path \\$MasterHost\D$){
...
$MasterISSHandlerVersion = (Get-Command \\$MasterHost\D$\WebServer\DBBWEB\bin\dbbHandler2005.dll).FileVersionInfo.FileVersion
$MasterScaleServiceVersion = (Get-Command \\$MasterHost\D$\WebServer\ScaleService\dbbScaleService.exe).FileVersionInfo.FileVersion
...
try {
Clear-Variable CoreIssueRequest
